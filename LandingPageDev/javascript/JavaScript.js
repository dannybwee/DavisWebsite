$(document).ready(function(){
	...
	if($("#category").val() === 'items') {
      ...
      var itemGet = "ajax/itemlettersearch.php?key=" + $(this).text();
			$.get(itemGet, function(response) {
				populateList(response);
			});
		});
	} else if ($("#category").val === 'locations') {
    var locationGet = "ajax/locationlettersearch.php?key=" + $(this).text();
		$(".letter").click(function() {
			$.get(locationGet, function(response) {
				populateList(response);
			});
		});
	}
